...
#include "src/printcounter.h"

#if ENABLED(PRINTCOUNTER)
  #include "src/duration_t.h"
#endif
  ...
  #include "src/mesh_bed_leveling.h"
#endif

#if ENABLED(FWRETRACT)
  #include "src/fwretract.h"
#endif

#if ENABLED(POWER_LOSS_RECOVERY)
  #include "src/power_loss_recovery.h"
#endif
...
#else
  #include "src/ultralcd_impl_HD44780.h"
...
uint16_t max_display_update_time = 0;
  ...
  int16_t lcd_preheat_hotend_temp[2], lcd_preheat_bed_temp[2], lcd_preheat_fan_speed[2];
    ...
    #include "src/stepper_dac.h" //was dac_mcp4728.h MarlinMain uses stepper dac for the m-codes
 ...
 */
   ...
   * PREHEAT submenu items
   */
  void preheat_set_temp_menu(){       // @mmusicoc new submenu
    START_MENU();
    MENU_BACK(MSG_PREHEAT);
  ...
  /*
   * CONTROL > BED LEVEL functions **********************************************************************************************************************************
   */
   
  // @mmusicoc
//  #define HAS_ANY_BEDLEVEL ENABLED(BLTOUCH) || ENABLED(TMC_Z_CALIBRATION) || ENABLED(AUTO_BED_LEVELING_UBL) || ENABLED(LCD_BED_LEVELING) || ENABLED(HAS_LEVELING) || ENABLED(LEVEL_BED_CORNERS) || ENABLED(MESH_BED_LEVELING) || ENABLED(ENABLE_LEVELING_FADE_HEIGHT) || ENABLED(DELTA_CALIBRATION_MENU)
    ...
    /*
     *  CONTROL > BED LEVELING (case Delta)
     */
  ...
  /*
   * LEVELING BED submenu
   */
  void lcd_bedlevel_menu(){
    START_MENU();
    MENU_BACK(MSG_HOME_MENU);
    if (planner.movesplanned() || IS_SD_PRINTING) {
    ...
    #if ENABLED(BLTOUCH)                                        // ITEM: BLTouch Self-Test and Reset *********************************
      MENU_ITEM(gcode, MSG_BLTOUCH_SELFTEST, PSTR("M280 P" STRINGIFY(Z_PROBE_SERVO_NR) " S" STRINGIFY(BLTOUCH_SELFTEST)));
      ...
      MENU_ITEM(gcode, MSG_TMC_Z_CALIBRATION, PSTR("G28\nM915"));   // ITEM: TMC Z Callib *********************************
    #endif

    #if ENABLED(AUTO_BED_LEVELING_UBL)                              // ITEM: Bed levelling *********************************
    ...
    #if ENABLED(DELTA_CALIBRATION_MENU) || ENABLED(DELTA_AUTO_CALIBRATION)        // ITEM: Delta callibration *********************************
    ...
    // END OF Bed leveling / callibrations -----------------------------------------------------------------------------------------------------------------

    /*
     * CONTROL > MOVE AXIS submenu items *******************************************************************************************************************************
     */
  ...
  void lcd_move_e() { _lcd_move_e(); }
    ...
    /*
     * CONTROL > MOVE AXIS > "Move Xmm" > "Move XYZ" submenu items
     */
  ...
  /*
   * CONTROL > ANDVANCE MODE (CHANGE FILAMENT) items ********************************************************************************************************************************************
   */
  #if ENABLED(ADVANCED_PAUSE_FEATURE)

    /*
     * CONTROL > CHANGE FILAMENT > "Change/Unload/Load Filament" submenu
     */
    ...
    /*
     * CONTROL > CHANGE FILAMENT submenu
     */
    #if E_STEPPERS > 1 || ENABLED(FILAMENT_LOAD_UNLOAD_GCODES)
      void lcd_change_filament_menu() {
        START_MENU();
  ...
  static void lcd_factory_settings() {
    ...
    /* 
     *  CONFIGURE > TEMPERATURE submenu items *******************************************************************************************************************************
     */
    ...
    /*
     * CONFIGURE > TEMPERATURE submenu
     */
  void lcd_control_temperature_menu() {         // @mmusicoc
    START_MENU();
    MENU_BACK(MSG_CONFIGURE);

   // Temps and fans control moved to preheat menu
  ...
  void lcd_control_motion_menu() {
    ...
    /*
     * CONFIGURE > DRIVER CURRENT CONTROL submenu items
     */
  #if ENABLED(DAC_STEPPER_CURRENT)
  ...
  /*
   *  End of CONFIGURE submenu items
   */
   ...
   */
    ...
    /*
     * ABOUT PRINTER > THERMISTORS submenu
     */
    void lcd_info_thermistors_menu() {
      if (use_click()) { return lcd_goto_previous_menu(); }
      START_SCREEN();
      #define THERMISTOR_ID TEMP_SENSOR_0
      #include "src/thermistornames.h"
 ...
 */
  ...
  void lcd_homing_menu(){  //##################### HOMING MENU ##########################         //@mmusicoc full function created from code in lcd_control_menu
    START_MENU();
    MENU_BACK(MSG_MAIN);
    
    MENU_ITEM(submenu, MSG_BED_LEVELING, lcd_bedlevel_menu); // ITEM: Bed leveling functions *********************************
  ...
  void lcd_preheat_menu(){  //##################### PREHEAT MENU ##########################         //@mmusicoc full new menu
    static byte material;
    START_MENU();
    MENU_BACK(MSG_MAIN);
    if ((thermalManager.target_temperature_bed == PREHEAT_1_TEMP_BED) or (thermalManager.target_temperature[0] == PREHEAT_1_TEMP_HOTEND)) material = 1;
    else if ((thermalManager.target_temperature_bed == PREHEAT_2_TEMP_BED) or (thermalManager.target_temperature[0] == PREHEAT_2_TEMP_HOTEND)) material = 2;
    else material = 0;

    MENU_ITEM(submenu, MSG_SET_TEMP_MENU, preheat_set_temp_menu);
  
    if (!thermalManager.target_temperature_bed){
      MENU_ITEM(function, MSG_PREHEAT_1_BEDONLY, lcd_preheat_m1_bedonly);
      MENU_ITEM(function, MSG_PREHEAT_2_BEDONLY, lcd_preheat_m2_bedonly);
    }
    else if (!thermalManager.target_temperature[0]){
      if (material != 2) MENU_ITEM(function, MSG_PREHEAT_1_END, lcd_preheat_m1_e0_only);
      if (material != 1) MENU_ITEM(function, MSG_PREHEAT_2_END, lcd_preheat_m2_e0_only);
    }
    MENU_ITEM(function, MSG_PREHEAT_1_ALL, lcd_preheat_m1_all);
    MENU_ITEM(function, MSG_PREHEAT_2_ALL, lcd_preheat_m2_all);
    if (thermalManager.target_temperature_bed or thermalManager.target_temperature[0]){
      MENU_ITEM(function, MSG_COOLDOWN, lcd_cooldown);
    }
    END_MENU();
  }

  void lcd_control_menu() { //##################### CONTROL MENU ##########################         //@mmusicoc edited all menu
    START_MENU();
    MENU_BACK(MSG_MAIN);  // ITEM: back to Main *****************************

    if (planner.movesplanned() || (IS_SD_PRINTING)){
      MENU_ITEM_EDIT(int3, MSG_SPEED, &feedrate_percentage, 10, 999);     // ITEM: Tune fedrate *********************************
    }
    else {
      #if ENABLED(DELTA)
        if (all_axes_homed())
      #endif
      MENU_ITEM(submenu, MSG_MOVE_AXIS_MENU, lcd_move_menu);     // ITEM: Move axis *********************************
    }
    
    MENU_ITEM(submenu, MSG_FLOW_MENU, lcd_flow_menu);         // ITEM: Tune flow on each extruder *********************************

    #if ENABLED(ADVANCED_PAUSE_FEATURE)                       // ITEM: Change filament *********************************
      if (!IS_SD_FILE_OPEN) {
        #if E_STEPPERS == 1 && !ENABLED(FILAMENT_LOAD_UNLOAD_GCODES)
          if (thermalManager.targetHotEnoughToExtrude(active_extruder))
    ...
    if (!planner.movesplanned() && !(IS_SD_PRINTING)){
      MENU_ITEM(gcode, MSG_DISABLE_STEPPERS, PSTR("M84"));        // ITEM: Disable steppers *********************************

      #if HAS_POWER_SWITCH                                        // ITEM: Switch power on/off *********************************
        if (powersupply_on)
    ...
    END_MENU();   // CONTROL MENU end ------------------------------------------------------------------------------------------------------------
  }

  void lcd_configure_menu() {     //##################### CONFIGURE MENU ##########################         //@mmusicoc edited all menu
    START_MENU();
    MENU_BACK(MSG_MAIN);
    
    MENU_ITEM(submenu, MSG_MOTION, lcd_control_motion_menu);                    // ITEM: tune steps/mm, max vel, accel and jerk

    #if DISABLED(NO_VOLUMETRICS) || ENABLED(ADVANCED_PAUSE_FEATURE)
      MENU_ITEM(submenu, MSG_FILAMENT, lcd_control_filament_menu);              // ITEM: filament configuration
    #elif ENABLED(LIN_ADVANCE)
      MENU_ITEM_EDIT(float52, MSG_ADVANCE_K, &planner.extruder_advance_K, 0, 999);    // ITEM: linear advance parameters
    #endif
    ...
    #if ENABLED(FWRETRACT)
      MENU_ITEM(submenu, MSG_RETRACT, lcd_control_retract_menu);                  // ITEM: 
    #endif
    
    #if ENABLED(DAC_STEPPER_CURRENT)
      MENU_ITEM(submenu, MSG_DRIVE_STRENGTH, lcd_dac_menu);                       // ITEM: set motor current
    #endif
    
    #if HAS_MOTOR_CURRENT_PWM
      MENU_ITEM(submenu, MSG_DRIVE_STRENGTH, lcd_pwm_menu);                       // ITEM: set motor current
    #endif

    #if ENABLED(BLTOUCH)
      MENU_ITEM(submenu, MSG_BLTOUCH, bltouch_menu);                              // ITEM: BLTouch menu
    #endif

    #if ENABLED(EEPROM_SETTINGS)                                                  // ITEM: EEPROM Save and restore configuration
      MENU_ITEM(function, MSG_STORE_EEPROM, lcd_store_settings);
      MENU_ITEM(function, MSG_LOAD_EEPROM, lcd_load_settings);
    #endif

    #if ENABLED(EEPROM_SETTINGS) && DISABLED(SLIM_LCD_MENUS)                      // ITEM: Load EEPROM parameters
      MENU_ITEM(submenu, MSG_INIT_EEPROM, lcd_init_eeprom_confirm);
    #endif

    MENU_ITEM(function, MSG_RESTORE_FAILSAFE, lcd_factory_settings);              // ITEM: Factory reset parameters

    END_MENU();   // CONFIGURE MENU end --------------------------------------------------------------------------------------------------------------------------------
  }

    void lcd_main_menu() {      // ####################### MAIN MENU #################################      @mmusicoc custom main menu
    START_MENU();
    MENU_BACK(MSG_WATCH);

    #if ENABLED(SDSUPPORT)                                // ITEM: Print from SD *********************************
      if (card.cardOK) {
        if (card.isFileOpen()) {
          if (card.sdprinting)
            MENU_ITEM(function, MSG_PAUSE_PRINT, lcd_sdcard_pause);
    ...
    MENU_ITEM(submenu, MSG_HOME_MENU, lcd_homing_menu);     // ITEM: Homing menu *********************************

    MENU_ITEM(submenu, MSG_PREHEAT, lcd_preheat_menu);      // ITEM: Temperature menu *********************************

    MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);      // ITEM: Control (when not printing) *********************************

    if (!planner.movesplanned() && !(IS_SD_PRINTING)){
      #if ENABLED(SDSUPPORT) && ENABLED(MENU_ADDAUTOSTART)        // ITEM: Autostart *********************************
        MENU_ITEM(function, MSG_AUTOSTART, lcd_autostart_sd);
      #endif
      MENU_ITEM(submenu, MSG_CONFIGURE, lcd_configure_menu);  // ITEM: Configure *********************************
    }
    
    #if ENABLED(CUSTOM_USER_MENUS)
      MENU_ITEM(submenu, MSG_USER_MENU, _lcd_user_menu);    // ITEM: Custom menu *********************************
    #endif
    
    #if HAS_DEBUG_MENU                                      // ITEM: Debug Menu  *********************************
      MENU_ITEM(submenu, MSG_DEBUG_MENU, lcd_debug_menu);
    #endif

    #if ENABLED(MENU_ITEM_CASE_LIGHT)                       // ITEM: Set Case light on/off/brightness *********************************
    ...
    #if ENABLED(LED_CONTROL_MENU)                           // ITEM: Led control *********************************
      MENU_ITEM(submenu, MSG_LED_CONTROL, lcd_led_menu);
    #endif

    if (!planner.movesplanned() && !(IS_SD_PRINTING)){
      #if ENABLED(LCD_INFO_MENU)
        MENU_ITEM(submenu, MSG_INFO_MENU, lcd_info_menu);     // ITEM: Info *********************************
      #endif
    }

    END_MENU();   // MAIN MENU end --------------------------------------------------------------------------------------------------------------------------
  }
